...
- `점심 메뉴 추천을 시작합니다.` 문구를 출력한다.
- `코치의 이름을 입력해주세요. (, 로 구분)`  을 출력한다
- 코치의 이름은 최소 2글자, 최대 4글자이다.
  - 2이상 4이하의 이름이 아니라면 에러를 출력한다. → `[ERROR] 코치의 이름은 2글자 이상 4글자 이하여야 합니다.`
- 코치는 최소 2명 최대 5명이여야 한다.
  - 코치의 수가 2이상 5이하가 아니라면 에러를 출력한다. → `[ERROR] 코치는 최소 2명 최대 5명입니다.`

### 2. 코치 별로 못 먹는 메뉴를 입력 받는다.

- `[코치이름] (이)가 못 먹는 메뉴를 입력해주세요.` 를 출력한다.
- 못먹는 메뉴를 ,로 구분한다.
  - 못 먹는 메뉴가 0개 이상 2개 이하가 아니라면 에러를 출력한다. → `[ERROR] 못먹는 음식은 0개 이상 2개 이하여야 합니다.`
  - 못 먹는 메뉴가 존재하지 않는 메뉴라면 에러를 출력한다. → `[ERROR] 못 먹는 메뉴가 추천할 수 있는 메뉴에 존재하지 않습니다.`

### 3. 메뉴를 추천한다.

- 월요일에 추천할 카테고리를 무작위로 정한다.
- 각 코치에게 월요일에 해당하는 카테고리에서 메뉴를 추천한다.
- 위 과정을 화~금요일까지 반복한다.
- 각 코치에게 한 주에 중복되지 않는 메뉴를 추천해야한다.
- 한 주에 같은 카테고리는 최대 2회까지만 추천가능하다.
